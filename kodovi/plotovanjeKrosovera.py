...
#    matricaZastupljenosti= numpy.zeros([brojGeneracija,64,15], dtype=int)
    ...
    for x in range (brojGeneracija):
        for y in range (64):
            ...
            a1[x][y]=a
            b1[x][y]=b
    #plotovanje strategije u generaciji:
...
def svesve():
    ...
    workbook = xlsxwriter.Workbook("rekombinacije harmony(0.7) " +".xlsx")
    ...
    workbook = xlsxwriter.Workbook("greske rekombinacije harmony(0.07)"+".xlsx")
    worksheet = workbook.add_worksheet()
    row = 0
    for col, data in enumerate(g):
        worksheet.write_column(row, col, data)
    workbook.close()
    #bez ifa jer testiram
